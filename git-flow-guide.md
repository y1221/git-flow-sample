...
### 2-3. メインブランチとサポートブランチの違い

| 観点             | メインブランチ                           | サポートブランチ                        |
| ---------------- | ---------------------------------------- | --------------------------------------- |
| **直接コミット** | 禁止（PRのターゲットになる）             | 可能（作業場所となる）                  |
| **存在期間**     | プロジェクト全期間にわたって存在し続ける | 目的完了後に削除する                    |
| **タグ**         | `main` にのみリリースタグを付与する      | 付与しない                              |
| **命名**         | 固定名（`main` / `develop`）             | 種別+内容の命名規則（`feature/xxx` 等） |
| **ブランチ保護** | Branch protection rules で保護する       | Rulesets で命名規則のみ強制する         |
...
タグのバージョン番号は **セマンティックバージョニング** の規約に従って付けます。
...
| 番号      | 上げるタイミング                                                  | リセット              |
| --------- | ----------------------------------------------------------------- | --------------------- |
| **MAJOR** | 後方互換性のない変更を加えたとき（APIの廃止・大幅な仕様変更など） | MINOR・PATCHを0に戻す |
| **MINOR** | 後方互換性を保ちながら新機能を追加したとき                        | PATCHを0に戻す        |
| **PATCH** | 後方互換性を保ちながらバグを修正したとき                          | そのまま加算          |

**git-flowのブランチ種別との対応:**

| ブランチ    | バージョン更新の目安                  | 例                  |
| ----------- | ------------------------------------- | ------------------- |
| `release/*` | 新機能追加なら MINOR を上げる         | `v1.0.0` → `v1.1.0` |
| `release/*` | 破壊的変更を含むなら MAJOR を上げる   | `v1.1.0` → `v2.0.0` |
| `hotfix/*`  | バグ修正のみのため常に PATCH を上げる | `v1.0.0` → `v1.0.1` |

> **初期開発について**: `v0.x.x` の間は仕様が安定していないとみなし、MAJORを0に据え置くことが慣例です。`v1.0.0` は「初めて公開APIとして安定した」ことを意味します。
...
## 12. 補足：GitHub Actions によるCIチェック

コミットメッセージの接頭辞ルールはGitHubのブランチ保護設定では対応できないため、GitHub Actionsで検証します。以下の4つのチェックを設定します。

---

### チェック①：コミットメッセージの接頭辞形式を検証

全サポートブランチへのpushに対して、Conventional Commits規約に沿った接頭辞が付いているかを検証します。
          ...
          git log origin/develop..HEAD --pretty=format:"%s" | while read msg; do
            if ! echo "$msg" | grep -qE "$VALID_PREFIXES"; then
              ...
              if ! echo "$msg" | grep -qE "^Merge "; then
                echo "ERROR: 不正なコミットメッセージ: \"$msg\""
                echo "形式: <接頭辞>: <内容>  例) feat: ログイン機能を追加"
                exit 1
              fi
            fi
          done
...
### チェック②：`release/*` / `hotfix/*` での `feat:` コミットを検出

リリース準備・緊急修正ブランチへの新機能追加（git-flowルール違反）を検知します。

```yaml
# .github/workflows/check-branch-commit-rule.yml
name: Check Branch Commit Rule
...
PRタイトルはマージコミットのメッセージになるため、コミットメッセージと同様に形式を統一します。
    ...
    steps:
      - name: PRタイトルの形式を検証
        ...
        run: |
          ...
          if ! echo "$PR_TITLE" | grep -qE "$VALID_PREFIXES|$MERGE_PATTERN"; then
            echo "ERROR: PRタイトルが規約に沿っていません: \"$PR_TITLE\""
            echo "形式: <接頭辞>: <内容>  例) feat: ログイン機能を追加"
            exit 1
          fi
```
...
ブランチ種別に応じたマージ先（ベースブランチ）が正しいかを確認し、誤ったPRを検知します。

| ブランチ    | 正しいマージ先          |
| ----------- | ----------------------- |
| `feature/*` | `develop` のみ          |
| `release/*` | `main` または `develop` |
| `hotfix/*`  | `main` または `develop` |
  ...
  check-target:
    ...
    steps:
      - name: PRのマージ先ブランチを検証
        ...
        run: |
          if echo "$HEAD" | grep -qE "^feature/"; then
            if [ "$BASE" != "develop" ]; then
              echo "ERROR: feature/* ブランチのマージ先は develop でなければなりません。(現在: $BASE)"
              exit 1
            fi
          elif echo "$HEAD" | grep -qE "^release/"; then
            if [ "$BASE" != "main" ] && [ "$BASE" != "develop" ]; then
              echo "ERROR: release/* ブランチのマージ先は main または develop でなければなりません。(現在: $BASE)"
              exit 1
            fi
          elif echo "$HEAD" | grep -qE "^hotfix/"; then
            if [ "$BASE" != "main" ] && [ "$BASE" != "develop" ]; then
              echo "ERROR: hotfix/* ブランチのマージ先は main または develop でなければなりません。(現在: $BASE)"
              exit 1
            fi
          fi
```
...
| チェック                   | トリガー                          | 検証内容                               |
| -------------------------- | --------------------------------- | -------------------------------------- |
| ① コミットメッセージ形式   | サポートブランチへのpush          | 接頭辞が規約に沿っているか             |
| ② ブランチ別コミットルール | `release/*` / `hotfix/*` へのpush | `feat:` コミットが混入していないか     |
| ③ PRタイトル形式           | `develop` / `main` へのPR         | タイトルが規約に沿っているか           |
| ④ PRマージ先検証           | 全PRオープン時                    | マージ先がgit-flowルールに沿っているか |
